    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{ background: "#222", padding: 10, borderRadius: 6 }}>
          <div className="text-style3 text-size4">
...
const StockDetailsCom = ({ ticker }: { ticker: string }) => {
  ...
  const stockFetch = useStockByTickerAndPeriod(
    ticker,
    new Date(from),
    new Date(to),
  );
  ...
  const data = useMemo<StockDetailsData | null>(() => {
    if (ticker === "" || !stockFetch.data || !signalsFetch.data) return null;
    return {
  ...
  const renderContent = () => {
    return (
      <>
        ...
        <div className={styles.chartHolder}>{data && createChart(data)}</div>
      </>
    );
  };
